...
async function clearAds(page, { timeoutMs = 45_000 } = {}) {
  ...
  log.warn('광고가 끝나지 않았습니다 — 이 장면은 건너뜁니다 (광고 화면을 싣지 않기 위해).');
  return false;
}
 ...
 */
export async function captureFrames(videoId, seconds, { title = '', candidates = 5, headless = true } = {}) {
  ...
  try {
    ...
    for (const sec of seconds) {
      ...
      await page.goto(`https://www.youtube.com/watch?v=${videoId}&t=${Math.round(sec)}s`, {
      ...
      const ready = await page
        .waitForFunction(
          (tt) => {
            ...
            return v && v.readyState >= 3 && v.videoWidth > 0 && Math.abs(v.currentTime - tt) < 10;
          },
          sec,
          { timeout: 60_000 }
        )
      ...
      if (!ready) {
        const err = await page.evaluate(
          () => document.querySelector('.ytp-error')?.innerText?.slice(0, 120) || null
        );
        log.warn(`${mmss(sec)} 구간을 불러오지 못했습니다${err ? ` (${err})` : ''} — 건너뜁니다.`);
        continue;
      }
      ...
      await page
        .waitForFunction(
          () => {
            const v = document.querySelector('video');
            return v && v.videoWidth >= 1280 && v.getBoundingClientRect().width >= 1280;
          },
          null,
          { timeout: 10_000 }
        )
        .catch(() => log.debug('고화질 전환이 확인되지 않았습니다 — 그대로 진행합니다.'));
      ...
      // 버퍼 안에서는 정지 상태 seek 이 수십 ms 로 끝난다.
      await page.evaluate(() => {
        const v = document.querySelector('video');
        v.muted = true;
        v.pause();
