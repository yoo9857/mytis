...
/**
 * 발행 간격을 지킨다 — **캡차를 만나지 않기 위해서다.**
 *
 * 티스토리는 연속 발행을 스팸으로 보고 DKAPTCHA(지도 캡차)를 띄운다. 캡차는 사람이
 * 풀어야 하므로, 그 순간 무인 실행이 끊긴다. 뚫는 것은 방법이 아니다 — 계정 정지
 * 사유이고, 그러면 블로그 자체가 사라진다.
 *
 * > 2026-08-05 실측: 한 시간 안에 4번 발행을 시도해 **4번 모두** 캡차가 떴다.
 * > HANDOVER 에도 "한 시간 반에 9건" 뒤 발생 기록이 있다.
 *
 * 그래서 **간격 자체를 코드가 지킨다.** 직전 발행에서 `minGapMinutes` 가 지나지
 * 않았으면 즉시 발행을 포기하고 **예약 발행**으로 돌린다(티스토리 기능이라 캡차와
...
/**
 * 간격이 모자라면 **cfg 를 예약 발행으로 바꿔** 캡차를 피한다.
 *
...
export function applyPacing(cfg, platform, blog, { force = false } = {}) {
  ...
  if (force) {
    log.warn(
      `직전 발행에서 ${gap.sinceMinutes}분밖에 지나지 않았습니다 (권장 ${minGap}분) — ` +
        '--now 가 있어 즉시 발행합니다. 캡차가 뜰 수 있습니다.'
    );
  ...
  log.warn(
    `직전 발행에서 ${gap.sinceMinutes}분밖에 지나지 않았습니다 (권장 간격 ${minGap}분). ` +
      `연속 발행 캡차를 피하려고 **${cfg.blog.reserveAfterMinutes}분 뒤 예약 발행**으로 돌립니다 — ` +
      '즉시 올리려면 --now 를 붙이세요.'
  );
