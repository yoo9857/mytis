...
**2단계 인증·기기 등록·틀린그림찾기가 뜨면 이 창에서 직접 통과해 주세요.** 최대 3분 기다립니다.
...
### 발행 중에 지도 문제(틀린그림찾기)가 떴다

연속 발행이 잦으면 티스토리가 "지도에 있는 ○○의 전체 명칭을 입력해주세요" 문제를 띄웁니다.
발행이 **스스로 처리합니다** — 문제 그림을 받아 라벨을 읽고(인식), 입력칸을 채우고(입력),
"답변 제출"을 누릅니다(제출). 틀리면 "새로 풀기"로 새 문제를 받아 최대 3회 시도합니다.

그래도 못 풀면 예전처럼 사람을 기다립니다. `--show` 로 창이 떠 있으면 바로 풀 수 있고,
답을 한 줄 적어 넣어도 대신 입력·제출합니다:

```powershell
Set-Content .tmp\wrong-picture-answer.txt "개방화장실" -Encoding utf8
```

판독만 따로 확인하려면 (발행하지 않습니다):

```powershell
npm run sec -- logs\shots60821-092641-publish-sec.png "지도에 있는 화장실의 전체 명칭을 입력해주세요"
```

짧은 시간에 시도가 잦으면 **카카오가 위젯 자체를 막습니다** (iframe 이 `Bad Request`).
이때는 지도도 입력칸도 없어 사람도 풀 수 없으므로 기다리지 않고, 레이어를 닫고
**5분 → 10분** 쉰 뒤 발행을 다시 누릅니다 (기본 2회). 그동안 에디터는 열린 채 유지되고
글은 자동 저장됩니다.

조절하는 값은 `.env.example` 의 `MONEYTI_SEC_*` · `MONEYTI_WRONG_PICTURE_WAIT` 를 보세요.
